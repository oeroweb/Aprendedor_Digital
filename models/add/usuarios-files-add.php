<?php 
  ...
  foreach ($usuariosList as $usuariosData) {    
    ...
    VALUES 
    (4, '$usuariosData[0]', '".utf8_encode(strtolower($usuariosData[1]))."','".utf8_encode(strtolower($usuariosData[2]))."', '".utf8_encode(strtolower($usuariosData[3]))."', '{$usuariosData[4]}', '".strtolower($usuariosData[5])."', '{$password_segura}','".utf8_encode(strtolower($usuariosData[6]))."', '".strtolower($usuariosData[7])."', '{$usuariosData[8]}', CURDATE(), 1)";

    $resultado = mysqli_query($db, $sql);      
    echo '<br>'; var_dump($sql); die();
  }
